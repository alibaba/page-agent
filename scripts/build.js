 ...
 *                    && npm run build:website -w @eb-agent/website
 *                    && npm run zip -w @eb-agent/ext
 *
 * 1. cleanup
...
tasks.push(
	{
		label: '@eb-agent/website',
		command: 'npm run build:website',
		cwd: join(rootDir, 'packages/website'),
	},
	{ label: '@eb-agent/ext', command: 'npm run zip', cwd: join(rootDir, 'packages/extension') }
)
