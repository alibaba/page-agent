...
exec(`${cmd} "${url}"`, (err) => {
	if (err) console.error(`[eb-agent-mcp] Could not open browser: ${err.message}`)
})

// --- MCP server (stdio) ---

const mcpServer = new McpServer({ name: 'eb-agent', version })
...
mcpServer.registerTool(
	'get_status',
	{
		description: 'Check the current status of the EBAgent hub.',
	},
	async () => ({
		content: [
			{
...
console.error('[eb-agent-mcp] MCP server ready (stdio)')
