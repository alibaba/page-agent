...
const enUS = {
	ui: {
		panel: {
			...
			userAnswerPromptOptions: 'Pick an option above, or type your answer',
			reflectionEval: 'Eval',
		...
		tools: {
			...
			identifyingImage: 'Identifying attached image...',
			done: 'Task done',
			clicked: 'Clicked element [{{index}}]',
			inputted: 'Inputted text "{{text}}"',
			selected: 'Selected option "{{text}}"',
			scrolled: 'Page scrolled',
			waited: 'Wait completed',
...
const zhCN = {
	ui: {
		panel: {
			...
			userAnswerPromptOptions: '点击上方选项，或输入回答',
			reflectionEval: '评估',
			reflectionMemory: '记忆',
			reflectionNext: '下一步',
			...
			imageAttached: '已添加图片',
			removeImage: '移除图片',
			defaultImageTask: '识别附带图片中的内容，然后决定下一步操作。',
